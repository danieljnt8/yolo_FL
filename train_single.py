...
def init_helper(hyp,  # path/to/hyp.yaml or hyp dictionary
          opt,
            device
          ):
    ...
    return train_loader,dataset,model
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    w = save_dir /"base"/'weights'  # weights dir
    (w.parent if evolve else w).mkdir(parents=True, exist_ok=True)  # make dir
    plot_path= save_dir /"base"/'plot'  # weights dir
    (plot_path.parent if evolve else plot_path).mkdir(parents=True, exist_ok=True)  # make dir
...
def prepare(opt, callbacks=Callbacks()):
    ...
    if not opt.evolve:
        #print('Inside1')
        test_dataloader,dataset,model = init_helper(opt.hyp, opt, device)
        if WORLD_SIZE > 1 and RANK == 0:
            LOGGER.info('Destroying process group... ')
            dist.destroy_process_group()
    
    return test_dataloader,dataset,model
...
if __name__ == "__main__":
    ...
    train_loader,dataset,model= prepare(opt) 
    
    train(opt.hyp, opt,device,callbacks=Callbacks(),trainloader = train_loader,model= model)